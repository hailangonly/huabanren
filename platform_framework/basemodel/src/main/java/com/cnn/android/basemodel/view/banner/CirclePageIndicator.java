...
import com.cnn.android.basemodel.view.banner.adapter.PageIndicator;
...
public class CirclePageIndicator extends View implements PageIndicator {
    ...
    private boolean mScrollableByClick = false;
    ...
    public CirclePageIndicator(Context context, AttributeSet attrs, int defStyle) {
        ...
        mScrollableByClick = a.getBoolean(R.styleable.CirclePageIndicator_scrollable_by_click, false);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mOrientation == HORIZONTAL) {
            ...
            shortPaddingBefore = getHeight() - getPaddingBottom();
        } else {
            longSize = getHeight();
        ...
        // fix: use the mSpace replace threeRadius(final float threeRadius =
        // mRadius * 3;)
        final float space = mRadius * 2.0f + mSpace;
        final float shortOffset = shortPaddingBefore - mRadius;
        float longOffset = longPaddingBefore + mRadius;
        if (mCentered) {
            longOffset += ((longSize - longPaddingBefore - longPaddingAfter) / 2.0f)
                    - (((count - 1) * space + mRadius * 2) / 2.0f);
        }
        ...
        for (int iLoop = 0; iLoop < count; iLoop++) {
            float drawLong = longOffset + (iLoop * space);
            if (mOrientation == HORIZONTAL) {
                dX = drawLong;
                dY = shortOffset;
            } else {
    ...
    public void setScrollableByClick(boolean enable) {
        mScrollableByClick = enable;
    }
